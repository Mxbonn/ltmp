...
parser.add_argument("--split", metavar="NAME", default="val", help="dataset split (default: validation)")
parser.add_argument(
    "--dataset-download",
    action="store_true",
    default=False,
...
parser.add_argument("-r", default=7, type=int, help="r size for the the token merging")
parser.add_argument(
...
def validate(args):
    ...
    # create model
    if args.model.startswith(("mp_", "pm_", "m_", "p_")):
        if args.model.startswith(("mp_", "pm_")):
            args.r = [args.r] * 12
        model = create_model(
            args.model,
    ...
    print("Time: ", batch_time.sum)
    eval_metrics = {"eval_top1": results["top1"], "eval_top5": results["top5"]}
    if not args.model.startswith("lt"):
        eval_metrics["eval_FLOPs"] = ltmp.utils.benchmark_flops_with_real_data(
            model, loader, device, args.batch_size, verbose=True
        )
    if has_wandb and args.log_wandb:
