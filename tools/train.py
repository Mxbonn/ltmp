...
group.add_argument("--val-split", metavar="NAME", default="val", help="dataset validation split")
group.add_argument(
    "--dataset-download",
    action="store_true",
    default=False,
...
group = parser.add_argument_group("learned threshold parameters")
group.add_argument("--reg-scale", type=float, default=10.0, help="lambda regularization parameter")
group.add_argument("--cls-scale", type=float, default=1.0, help="class loss scale parameter")
group.add_argument("--reduction-target", type=float, default=0.75, help="class loss scale parameter")
group.add_argument("--tau", type=float, default=0.5, help="Temperature scaling softmax for pruning")
group.add_argument("--debug", action="store_true", default=False, help="Debugging modDebugging mode")
...
group.add_argument("--opt", default="momentum", type=str, metavar="OPTIMIZER", help='Optimizer (default: "momentum")')
group.add_argument(
...
group.add_argument("--momentum", type=float, default=0.0, metavar="M", help="Optimizer momentum (default: 0.9)")
group.add_argument("--weight-decay", type=float, default=0, help="weight decay (default: 2e-5)")
group.add_argument(
...
group.add_argument(
    "--lr-base-size", type=int, default=1, metavar="DIV", help="base learning rate batch size (divisor, default: 256)."
)
group.add_argument(
    "--lr-base-scale",
    type=str,
...
group.add_argument("--epochs", type=int, default=1, metavar="N", help="number of epochs to train (default: 300)")
group.add_argument(
    "--epoch-repeats",
    type=float,
    default=0.0,
...
group.add_argument(
    "--warmup-epochs", type=int, default=0, metavar="N", help="epochs to warmup LR, if scheduler supports"
)
group.add_argument(
...
group.add_argument("--seed", type=int, default=random.randrange(2**31), metavar="S", help="random seed (default: 42)")
group.add_argument("--worker-seeding", type=str, default="all", help="worker seed mode (default: all)")
group.add_argument(
    "--log-interval", type=int, default=1, metavar="N", help="how many batches to wait before logging training status"
)
group.add_argument(
    "--recovery-interval",
    type=int,
...
def main():
    ...
    if utils.is_primary(args) and args.log_wandb:
        if has_wandb:
            wandb.init(project="LTMP", name=exp_name, config=args)
        else:
            _logger.warning(
    ...
    if len(args.lr) == 1:  # Only one learning rate means we're training ltm or ltp
        threshold_lrs = [args.lr[0], args.lr[0]]
        args.lr = args.lr[0]
    elif len(args.lr) == 2:  # Two learning rates means we're training ltpm
        threshold_lrs = args.lr
        args.lr = args.lr[0]
    elif (
        len(args.lr) == 3
    ):  # 3 learning rates means, lr for prune threshold, lr for merge threshold and lr for all other learnable weights.
        threshold_lrs = [args.lr[0], args.lr[1]]
        args.lr = args.lr[2]
        train_entire_model = True
    elif len(args.lr) > 3:
        _logger.error("There are more learning rates defined than there are number of parameter groups")
        threshold_lrs = args.lr[:2]
        args.lr = args.lr[:2]

    lr_param_groups = [
        {"params": [], "lr": threshold_lrs[0]},
        {"params": [], "lr": threshold_lrs[1]},
        {"params": [], "lr": args.lr},
    ]
    for n, p in model.named_parameters():
        if "masker" in n:
            ...
            if "prune" in n:
                lr_param_groups[0]["params"].append(p)
            elif "merge" in n:
                lr_param_groups[1]["params"].append(p)
        elif "norm" in n:
            p.requires_grad = False
        else:
            p.requires_grad = train_entire_model
            lr_param_groups[2]["params"].append(p)

    optimizer = create_optimizer_v2(lr_param_groups, **optimizer_kwargs(cfg=args))
    ...
    # split train in train and minival (2% of datset)
    generator = torch.Generator().manual_seed(2023)
    dataset_minival, dataset_train = torch.utils.data.random_split(dataset_train, [0.02, 0.98], generator=generator)
    dataset_minival = copy.deepcopy(dataset_minival)
    ...
    loader_eval_inference = create_loader(
        dataset_eval,
        input_size=data_config["input_size"],
        batch_size=1,
        is_training=False,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if epoch == (num_epochs - 1) and args.model.startswith("lt"):
                inference_model = create_model(f"inference_{args.model}")
                inference_model.load_state_dict(model.state_dict(), strict=False)
                inference_model.eval()
                eval_metrics["FLOPs"] = ltmp.utils.benchmark_flops_with_real_data(
                    inference_model, loader_eval_inference, device, 1000, verbose=True
                )
...
def train_one_epoch(
    ...
    reg_losses_m = utils.AverageMeter()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with torch.autograd.set_detect_anomaly(args.debug), amp_autocast():
            output = model(input)
            cls_loss = loss_fn(output, target)
            ## reg_loss
            if args.reg_scale > 0.0:
                flops_r = 0
                n = model.masks[0].shape[-1]
                ms = [1] + [torch.mean(m) for m in model.masks]
                for i in range(layers):
                    flops_r += (
                        (1 / layers)
                        * ((2 * (ms[i] * n) * d**2 + (ms[i] * n) ** 2 * d) + (4 * (ms[i + 1] * n) * d**2))
                        / (6 * n * d**2 + n**2 * d)
                    )
                reg_loss = (flops_r - args.reduction_target) ** 2
                loss = args.cls_scale * cls_loss + args.reg_scale * reg_loss
            else:
            ...
            if not args.distributed:
                ...
                reg_losses_m.update(reg_loss.item(), input.size(0))
                top1_m.update(acc1.item(), output.size(0))
                top5_m.update(acc5.item(), output.size(0))

            optimizer.zero_grad()
            if loss.isnan():
                _logger.info("Loss is Nan, skipping backprop this iteration.")
                continue
            loss.backward()
            optimizer.step()
            if args.reg_scale > 0.0:
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                ...
                if args.log_wandb and has_wandb:
                    log_dict = {
                        "Loss": losses_m.val,
                        "CLS_Loss": cls_losses_m.val,
                        "REG_Loss": reg_losses_m.val,
                        "Train Accuracy": top1_m.avg,
                        "x": num_updates * input.size(0),
                    }
                    log_dict.update({n: p.item() for n, p in model.named_parameters() if "masker" in n})
                    log_dict.update({"test_param": model.blocks[0].mlp.fc1.weight.flatten()[0]})
                    wandb.log(log_dict)
