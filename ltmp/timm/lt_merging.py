...
class LTMBlock(nn.Module):
    def __init__(
        ...
        self.attn = LTMAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, size, mask, viz) -> torch.Tensor:
        b, t, c = x.shape
        x_attn, metric = self.attn(self.norm1(x), size, mask)
...
class LTMVisionTransformer(VisionTransformer):
...
## Register all vision transformer variants to which you want to apply ltm here.
# ViT
@register_model
def ltm_vit_small_patch16_224(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, block_fn=LTMBlock, **kwargs)
    model = create_vision_transformer(
        LTMVisionTransformer, "vit_small_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model


@register_model
def ltm_vit_base_patch16_224(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, block_fn=LTMBlock, **kwargs)
    model = create_vision_transformer(
        LTMVisionTransformer, "vit_base_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model
...
# DEIT
@register_model
def ltm_deit_small_patch16_224(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, block_fn=LTMBlock, **kwargs)
    model = create_vision_transformer(
        LTMVisionTransformer, "deit_small_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model


@register_model
def ltm_deit_base_patch16_224(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, block_fn=LTMBlock, **kwargs)
    model = create_vision_transformer(
        LTMVisionTransformer, "deit_base_patch16_224", pretrained=pretrained, **model_kwargs
    )
    return model
